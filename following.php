<?php
/**
 * Pagina per visualizzare le persone seguite da un utente
...
// Otteniamo le persone seguite dall'utente
$followingQuery = "SELECT u.id, u.username, u.full_name, u.profile_pic, u.bio, f.created_at 
                  FROM users u 
                  JOIN friendships f ON u.id = f.following_id 
                  WHERE f.follower_id = {$profileId} AND f.status = 'accepted' 
...
if ($followingResult && $followingResult->num_rows > 0) {
    while ($follow = $followingResult->fetch_assoc()) {
        $following[] = $follow;
    }
}
            ...
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h5 class="card-title mb-0">
                        <i class="fas fa-users"></i> Persone seguite da <?php echo $user['full_name']; ?>
                        <?php if ($totalFollowing > 0): ?>
                            <span class="badge bg-light text-primary float-end"><?php echo $totalFollowing; ?></span>
                        <?php endif; ?>
                    </h5>
                </div>
                <div class="card-body p-0">
                    <?php if ($totalFollowing > 0): ?>
                        <ul class="list-group list-group-flush">
                            <?php foreach ($following as $follow): ?>
                                <?php
                                // Verifichiamo se l'utente corrente segue già questa persona
                                $isFollowing = false;
                                if ($currentUserId > 0) {
                                    if ($currentUserId == $follow['id']) {
                                        $isFollowing = true; // Sei tu stesso
                                    } else {
                                        $isFollowing = isset($relationships[$follow['id']]);
                                    }
                                }
                                
                                // Calcoliamo il tempo da cui segue
                                $followingSince = timeAgo(strtotime($follow['created_at']));
                                ?>
                                <li class="list-group-item">
                                    <div class="d-flex align-items-center">
                                        <a href="<?php echo SITE_URL; ?>/profile.php?id=<?php echo $follow['id']; ?>">
                                            <img src="<?php echo UPLOADS_URL; ?>/profile_pics/<?php echo $follow['profile_pic']; ?>" alt="<?php echo $follow['username']; ?>" class="rounded-circle me-3" width="60" height="60">
                                        </a>
                                        <div class="flex-grow-1">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <a href="<?php echo SITE_URL; ?>/profile.php?id=<?php echo $follow['id']; ?>" class="text-decoration-none">
                                                        <h5 class="mb-0"><?php echo $follow['full_name']; ?></h5>
                                                    </a>
                                                    <div class="text-muted">@<?php echo $follow['username']; ?></div>
                                                    <small class="text-muted">Seguito da <?php echo $followingSince; ?></small>
                                                </div>
                                                <?php if ($currentUserId > 0 && $currentUserId != $follow['id']): ?>
                                                    <div>
                                                        <button class="btn btn-sm <?php echo $isFollowing ? 'btn-secondary' : 'btn-primary'; ?> follow-button" data-user-id="<?php echo $follow['id']; ?>">
                                                            <?php if ($isFollowing): ?>
                                            ...
                                            <?php if (!empty($follow['bio'])): ?>
                                                <p class="mb-0 mt-2"><?php echo truncateText(htmlspecialchars($follow['bio']), 100); ?></p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </li>
                    ...
                    <?php else: ?>
                        <div class="p-4 text-center">
                            ...
                            <h5>Nessuna persona seguita</h5>
                            <p class="text-muted mb-0">
                                <?php if ($profileId == $currentUserId): ?>
                                    Non stai seguendo ancora nessuno. Cerca e segui altri utenti per riempire questa lista!
                                <?php else: ?>
                                    <?php echo $user['full_name']; ?> non sta seguendo ancora nessuno.
                                <?php endif; ?>
                            </p>
                        </div>
                    <?php endif; ?>
